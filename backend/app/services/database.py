    ...
    def __init__(self):
        ...
        if settings.AWS_ACCESS_KEY_ID and settings.AWS_SECRET_ACCESS_KEY:
            try:
                ...
                self.cases_table = dynamodb.Table(settings.DYNAMODB_TABLE_CASES)
                self.evidence_table = dynamodb.Table(settings.DYNAMODB_TABLE_EVIDENCE)
                print(f"Connected to DynamoDB Tables: {settings.DYNAMODB_TABLE_CASES}, {settings.DYNAMODB_TABLE_EVIDENCE}")
            except Exception as e:
                print(f"Failed to connect to DynamoDB: {e}")
        else:
            print("Running in Local Mode (No AWS Credentials found). Using 'local_db.json'")
